...
transcription=local/espnet_all
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    for dir in ${train_set} ${train_dev} ${eval_set}; do
        local/data_prep.sh ${data_dir} local/espnet_${dir} data/${dir}
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    echo "stage 3: Adding character embeddings and intonation type to data.json"
    # make scps for character embeddings and intonation types
    for name in ${train_set} ${train_dev} ${eval_set}; do
        ...
        cat data/${name}/wav.scp | awk -F' ' '{type=0;if(match($1,"TRIA00706")) type=1; printf "%s %s\n",$1,type}' > data/${name}/into_type.scp
    done
    # update json
    for name in ${train_set} ${train_dev} ${eval_set}; do
        ...
        # update the json file and remove temporary files
